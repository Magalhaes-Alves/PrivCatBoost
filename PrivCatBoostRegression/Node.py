...
class Node:
    def __init__(
        ...
        hyperparam,
        parent=None,
        gain_function=Gain.gainContinuous,
        split_function=Gain.splitContinuous,
    ):
        self.parent = parent
    ...
    def calcLeafValue(self, gradient, tree, e_leaf, v, learning_rate):
        ...
        v_t = -1 * (
            np.sum(gradient) / (gradient.shape[0] + self.hyperparameters.regularization)
        )

        # Geometric Leaf Cliping
        if v_t != 0.0:
            eval = (
                self.hyperparameters.gl
                * (1 - self.hyperparameters.learningRate) ** (tree)
            ) / fabs(v_t)
            v_t = v_t * min(1, eval)
    ...
    def fit(self, e_leaf, e_enleaf, tree, depth=0):
        ...
        if len(xi) == 0:
            print(xi)
            print(yi)
        if depth == max_depth or xi.shape[0] < min_sample_size or xi.empty:
        ...
        childs = self.split_function(
            xi, yi, feature, split, feature in self.hyperparameters.categoricalFeatures
        )

        for k, i in childs.items():
            if i[0].empty:
                self.classification = self.calcLeafValue(
                    yi, tree, e_leaf, v, learning_rate
                )
                self.__desaloc_data()
                return depth
        ...
        for key, child in self.children.items():
            max_depth_child = max(
                max_depth_child, child.fit(e_leaf, e_enleaf, tree, depth + 1)
            )
    ...
    def addChildren(self, key, xi, yi):
        self.children[key] = Node(
            xi=xi,
            yi=yi,
            hyperparam=self.hyperparameters,
            parent=self,

...
class PrivCatBoostClassification(ClassifierMixin, BaseEstimator):
    ...
    def preprocessing(self, data, target):
        ...
        self.hyperparameters.categoricalFeatures = [
            self.data.columns.get_loc(i) for i in self.categorical_features
        ]
    ...
    def fit(self, data, target):
        ...
        for tree in range(1, self.total_trees + 1):
            ...
            n_samples = (
                self.data.shape[0] * learning_rate * ((1 - learning_rate) ** t_e)
            )
            n_samples = round(
                n_samples / (1 - ((1 - learning_rate) ** self.trees_in_ensemble))
            )
            ...
            self.hyperparameters.Order = np.zeros(
                shape=(self.data.shape[1], select_index.shape[0])
            )
